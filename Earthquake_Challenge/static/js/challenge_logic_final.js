...
let overlays = {
  ...
  "Tectonic Places": tectonicPlates,
  "Major Earthqaukes": majorEarthquakes,
};
  ...
  d3.json(
    ...
    function styleInfo(feature) {
      return {
        opacity: 1,
        fillOpacity: 1,
        fillColor: setMajorColor(feature.properties.mag),
        color: "#000000",
        radius: getRadius(feature.properties.mag),
        stroke: true,
        weight: 0.5,
    ...
    function setMajorColor(magnitude) {
...
legend.onAdd = function () {
  ...
  for (var i = 0; i < magnitudes.length; i++) {
    ...
    const text=magnitudes[i] + (magnitudes[i + 1] ? "&ndash;" + magnitudes[i + 1] + "<br>" : "+");
    const s = `<i style="background: ${colors[i]};"></i> ` + text;
    // div.innerHTML += s;
    div.innerHTML += `<div style="display: flex;"><div style="width: 25px; background: ${colors[i]}; margin-right: 5px;"></div><div>${text}</div></div>`;
  }
  return div;
};
...
d3.json(
  ...
  // Creating a GeoJSON layer with the retrieved data.
  L.geoJson(data, {
    // We set the style for each line
    style: setStyle,

    // We create a popup for each Line String to display
    onEachFeature: function (feature, layer) {
      layer.bindPopup(
        "Name: " +
